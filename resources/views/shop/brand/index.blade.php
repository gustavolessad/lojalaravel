...
@section('content')
    ...
    <h1 class="text-2xl font-bold text-gray-900 mb-4">Marcas</h1>
    ...
    @else
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            @foreach ($brands as $brand)
                <a href="{{ $brand->url }}"
                   class="group flex flex-col items-center justify-center gap-3 p-5 bg-white border border-gray-100 rounded-2xl hover:border-gray-700 transition-all">
                    ...
                    {{-- Nome --}}
                    <span class="text-sm font-medium text-gray-700 group-hover:text-black text-center transition-colors">
                        {{ $brand->name }}
                    </span>
                </a>
            @endforeach
