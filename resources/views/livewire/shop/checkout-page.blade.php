<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            ══════════════════════════════════════════════════════ --}}
            @if ($step === 0)
            <div wire:key="step-0" class="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            ...
            ══════════════════════════════════════════════════════ --}}
            @elseif ($step === 1)
            <div wire:key="step-1" class="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <h3 class="text-sm font-semibold text-black">Entrega</h3>
                <p class="text-xs text-gray-500">Selecione um endereço</p>
                </div>
                ...
                <div class="mt-4">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-sm font-semibold text-black">Opções de frete</h3>
                        <p class="text-xs text-gray-500">Selecione uma opção</p>
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <div wire:click="$set('selectedShippingIndex', {{ $idx }})"
                        class="flex items-center gap-3 p-3 shadow-xs border rounded-xl cursor-pointer transition-all
        ...
        {{-- ══════════════════════════════════════════════════════
            ...
            ══════════════════════════════════════════════════════ --}}
        @elseif ($step === 2)
        <div wire:key="step-2" class="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
            <div class="flex items-center gap-3">
                <h2 class="text-sm font-semibold text-black flex-1">Forma de pagamento</h2>
                <button type="button" wire:click="backTo(1)" class="text-sm text-indigo-600 hover:text-indigo-800">← Entrega</button>
            </div>

            {{-- Cupom de desconto --}}
            <div x-data="{ open: @js((bool) $this->cart?->coupon_code) }"
                ...
                <button type="button" @click="open = !open"
                    ...
                    @if ($this->cart?->coupon_code)
                        ...
                        <span class="truncate flex-1 text-green-700 font-semibold">{{ $this->cart->coupon_code }}</span>
                    @else
                        ...
                        <span class="truncate flex-1 text-gray-700">Cupom de desconto</span>
                    @endif
                ...
                <div x-show="open" x-collapse class="px-4 pb-4 border-t border-gray-100 pt-3">
                    @if ($this->cart?->coupon_code)
                        <div class="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-3 h-[38px]">
                            <div class="flex items-center gap-1.5 min-w-0">
                                <span class="font-mono font-semibold text-green-700 text-xs truncate">{{ $this->cart->coupon_code }}</span>
                                <span class="text-xs text-green-600 shrink-0">- R$ {{ number_format($this->cart->coupon_discount, 2, ',', '.') }}</span>
                            </div>
